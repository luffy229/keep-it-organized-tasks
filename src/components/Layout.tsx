...
const Layout = ({ children }: LayoutProps) => {
  ...
  return (
    <div className="min-h-screen transition-all duration-300" style={getBackgroundStyle()}>
          ...
          <div className="flex justify-between items-center h-14 sm:h-16">
            ...
            <div className="flex items-center space-x-2 sm:space-x-6">
              <nav className="flex space-x-1">
                <Link
                  to="/"
                  className={`px-2 sm:px-4 py-2 rounded-xl text-xs sm:text-sm font-medium transition-all duration-300 ${
                    location.pathname === '/'
              ...
              {user && (
                <div className="flex items-center space-x-2 sm:space-x-4 border-l border-gray-200 pl-2 sm:pl-6">
                  <div className="flex items-center space-x-2 sm:space-x-3 bg-white/60 rounded-xl px-2 sm:px-4 py-2 shadow-sm">
                  ...
                  <button
                    onClick={logout}
                    className="p-1.5 sm:p-2 text-gray-600 hover:text-red-500 hover:bg-red-50 rounded-xl transition-all duration-200 hover:scale-110"
                    title="Logout"
                  >
      ...
      <main className="relative max-w-6xl mx-auto px-3 sm:px-4 md:px-6 lg:px-8 py-4 sm:py-8">
        {children}
      </main>
    </div>
  );
